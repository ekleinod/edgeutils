...
Dieser Installer f�hrt Sie durch die Installation. \
Sie k�nnen die Installation jederzeit abbrechen, indem Sie unten rechts auf $\"Abbrechen$\" klicken."
...
Sie k�nnen das Installationsprogramm beenden und danach $\"${LONGNAME}$\" starten."
...
Section "Startmen�-Eintrag" SEC_SM
	SetOverwrite on
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
	...
	!insertmacro MUI_DESCRIPTION_TEXT ${SEC_SM} "Der Eintrag im Startmen� sorgt daf�r, dass Sie $\"${LONGNAME}$\" bequem von Hand aus dem Startmen� starten k�nnen."
	!insertmacro MUI_DESCRIPTION_TEXT ${SEC_AS} "Dieser Eintrag sorgt daf�r, dass $\"${LONGNAME}$\" bei jedem Rechnerstart automatisch ausgef�hrt wird."
...
Function CopyExisting
	IfFileExists "$INSTDIR\${FILENAME}.jar" 0 +4
		MessageBox MB_OKCANCEL|MB_ICONQUESTION "${LONGNAME} ist bereits im Installationsverzeichnis vorhanden. Soll es �berschrieben werden?$\r$\n$\r$\nBei $\"Abbrechen$\" kann ein neues Verzeichnis ausgesucht werden." IDOK continue
		Abort
	continue:
FunctionEnd
